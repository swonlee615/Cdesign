...
int main()
{
    ...
    scanf("%d", &B);     // scanf 실행:  버퍼 변수(문자→엔터) 삭제  →  입력서식(%d→정수) '초기화' 변수자료형(정수→89) 매칭 O
                         // scanf 실행 성공:  정수 1개 89 입력/저장[%d 정수자료형 89(4 Bytes) B 주소값(&)을 시작으로 입력/저장]  →  '초기화' 변수(문자→엔터) 버퍼 유지
    printf("---Output---\n");
    printf("<%c>\n", B); // %c → 선언한 자료형크기 만큼 정수 1개 89 ASCII Code 변환 문자 1개 Y 출력

    int C;
    printf("---Input---\n");
    scanf("%c", &C);     // scanf 실행:  입력서식(%c→문자) 버퍼 변수자료형(문자→엔터) 매칭 O /// %c == 문자 1개
    ...
    int D;
    printf("---Input---\n");
    scanf("%c", &D);     // 입력서식(%c→문자) '초기화' 변수자료형(%c: 정수→89 X 문자→89엔터 O 인식) 매칭 O
